...
class LongCatVideoAvatarPipeline:
    ...
    def _get_t5_prompt_embeds(
        ...
        cache_key = (tuple(prompt), num_videos_per_prompt, max_sequence_length, str(dtype))
        if not hasattr(self, "_prompt_embed_cache"):
            self._prompt_embed_cache = OrderedDict()
        cached = self._prompt_embed_cache.get(cache_key)
        if cached is not None:
            self._prompt_embed_cache.move_to_end(cache_key)
            prompt_embeds, mask = cached
            print(f"[cache] prompt embeds hit {hash(cache_key)}", flush=True)
            return prompt_embeds.to(device=device, dtype=dtype, non_blocking=True), mask.to(device=device, non_blocking=True)
        cache_dir = Path(tempfile.gettempdir()) / "longcat_cache" / "prompt_emb"
        cache_dir.mkdir(parents=True, exist_ok=True)
        key_json = json.dumps(cache_key, sort_keys=True, separators=(",", ":"))
        cache_path = cache_dir / f"{hashlib.sha256(key_json.encode('utf-8')).hexdigest()}.pt"
        if cache_path.exists():
            try:
                prompt_embeds, mask = torch.load(cache_path, map_location="cpu")
                self._prompt_embed_cache[cache_key] = (prompt_embeds, mask)
                self._prompt_embed_cache.move_to_end(cache_key)
                print(f"[cache] prompt embeds disk hit {hash(cache_key)}", flush=True)
                return prompt_embeds.to(device=device, dtype=dtype, non_blocking=True), mask.to(device=device, non_blocking=True)
            except Exception as e:
                print(f"[cache] prompt disk cache read failed: {e}", flush=True)
        ...
        self._prompt_embed_cache[cache_key] = (prompt_embeds.detach().cpu(), mask.detach().cpu())
        self._prompt_embed_cache.move_to_end(cache_key)
        while len(self._prompt_embed_cache) > 8:
            self._prompt_embed_cache.popitem(last=False)
        try:
            torch.save(self._prompt_embed_cache[cache_key], cache_path)
        except Exception as e:
            print(f"[cache] prompt disk cache write failed: {e}", flush=True)
    ...
    def generate_ai2v(
        ...
        print(f"[timing] ai2v_encode_prompt={time.perf_counter() - t_stage:.2f}s", flush=True)
        t_stage = time.perf_counter()
        ...
        print(f"[timing] ai2v_prepare_latents={time.perf_counter() - t_stage:.2f}s", flush=True)
        t_stage = time.perf_counter()
        ...
        self._current_timestep = None
        print(f"[timing] ai2v_denoise={time.perf_counter() - t_stage:.2f}s", flush=True)
        ...
        output_video = self.vae.decode(latents, return_dict=False)[0]
        output_video = self.video_processor.postprocess_video(output_video)
        print(f"[timing] ai2v_vae_decode_post={time.perf_counter() - t_stage:.2f}s", flush=True)
